...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'LFAU', 'LFAU_C3', 'C3_ODConv', 'C2f_ODConv', 'PConv3', 'LFAU_EMA', 'LFAU_C3_EMA', 'C2f_DBB',
           ...
           'C3_UniRepLKNetBlock', 'C2f_UniRepLKNetBlock', 'C3_DRB', 'C2f_DRB', 'C3_DWR_DRB', 'C2f_DWR_DRB', 'HFAM', 'TFF', 'DynamicTFF', 'Add', 'CSP_EDLAN', 'CPAM',
           ...
           'C3_PPA', 'C2f_PPA', 'CSMHSA', 'SRFD', 'DRFD', 'CFC_CRB', 'SFC_G2', 'CGAFusion', 'CAFM', 'CAFMFusion', 'RGCSPELAN', 'LFAU_C3_CGLU', 'LFAU_CGLU', 'SDFM', 'PSFM',
...
class LFAU_C3(C3):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.Sequential(*(LFAU_Block(c_, c_) for _ in range(n)))

class LFAU(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(LFAU_Block(self.c, self.c) for _ in range(n))
...
class LFAU_Block_EMA(nn.Module):
    def __init__(self,
                 inc,
                 dim,
                 n_div=4,
...
class LFAU_C3_EMA(C3):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(LFAU_Block_EMA(c_, c_) for _ in range(n)))

class LFAU_EMA(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(LFAU_Block_EMA(self.c, self.c) for _ in range(n))
...
class DynamicTFF(nn.Module):
    def __init__(self, inc, channel):
        super(DynamicTFF, self).__init__()
        if channel != inc[0]:
            self.conv0 = Conv(inc[0], channel,1)
        self.conv1 =  Conv(inc[1], channel,1)
        self.conv2 =  Conv(inc[2], channel,1)
...
class CPAM(nn.Module):
    ...
    def __init__(self, ch=256):
        super().__init__()
        self.channel_att = CAM(ch)
        self.local_att = PAM(ch)
    def forward(self, x):
...
class LFAU_Block_CGLU(nn.Module):
    def __init__(self,
                 inc,
                 dim,
                 n_div=4,
...
class LFAU_C3_CGLU(C3):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(LFAU_Block_CGLU(c_, c_) for _ in range(n)))

class LFAU_CGLU(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(LFAU_Block_CGLU(self.c, self.c) for _ in range(n))
